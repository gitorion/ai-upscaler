...
**Prefilter defaults to `none` for SeedVR2** (the other models default to `light`). SeedVR2 is generative and trained on degraded input, so it reconstructs from the real degradation — pre-denoising tends to strip detail cues it would otherwise restore. Pass `--prefilter light/medium/heavy` to override if a particular source has extreme artifacts that benefit from a pre-clean.
