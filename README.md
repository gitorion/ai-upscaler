...
**Auto-calibrated batch size.** `SEEDVR2_BATCH` defaults to `auto`: on first use the pipeline binary-searches the largest 4n+1 batch your GPU can actually run, then caches it per resolution + block-swap setting under `~/ai-upscale/seedvr2/`. Because VRAM here depends on batch size and output resolution — not clip length, since `chunk_size` streams the video — a few seconds of footage is a faithful probe for a two-hour job. It costs about 4 probe runs once (a few minutes) and is then reused by every later run, including every episode of a season. Pin an integer to skip it, e.g. `SEEDVR2_BATCH=21`.

Why bother: batch length *is* the temporal-context window, so the largest that fits is the best motion quality available on your hardware. There is an upper limit worth knowing about though — a batch longer than a typical shot forces the model to reconcile across scene cuts, so `SEEDVR2_BATCH_MAX` caps the search at 49 (~2s). On 16GB you top out well below that anyway.
